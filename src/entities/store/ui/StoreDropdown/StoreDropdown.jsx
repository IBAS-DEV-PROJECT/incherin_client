...
// --- 내부 ---
import { filterTypeAtom } from '@features/store-filter/model/filterStore';
...
const StyledDropdownItem = styled.li(({ theme, $isSelected, $isHovered }) => ({
  ...
  backgroundColor:
    $isSelected || $isHovered ? theme.colors.lightBlue : 'transparent',
  cursor: 'pointer',
  display: 'flex',
  alignItems: 'center',
  borderRadius: '10px',
...
export function StoreDropdown({ options = [], placeholder = '정렬 선택' }) {
  ...
  const selectedLabel =
    options.find(opt => opt.value === filterType)?.label || placeholder;
